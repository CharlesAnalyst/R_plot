library(ggplot2)
setwd("/data6/revised_version/project/Figure4")

#scale_fill_manual(values=c("skyblue", "seagreen3", "darkorange2"))

###########group_total_data.txt
total_data <- read.table("Result.txt", sep = "\t", header = TRUE)
# names(total_data) <- c("score", "type", "group")
total_data$group <- factor(total_data$group, levels = c("L", "medium", "high"), ordered = TRUE)
...
m_l_data <- read.table("figure_c.TXT", header = TRUE)
# names(m_l_data) <- c("score", "a", "degree", "group")
names(m_l_data) <- c("group", "a", "degree", "score")
m_l_data$degree <- factor(m_l_data$degree, levels = c("L", "M", "H"), ordered = TRUE)
...
ggsave("Fiture_4C.pdf", width = 16, height = 4)
##########group_5utr-cds-stop-3utr.txt
feature_data <- read.table("Result.txt", sep = "\t", header = TRUE)
names(feature_data) <- c("a", "degree", "score",  "group")
feature_data$degree <- factor(feature_data$degree, levels = c("L", "M", "H"), ordered = TRUE)
feature_data$group <- factor(feature_data$group, levels = c("5-UTR", "CDS", "Stop_codon", "3-UTR"), ordered = TRUE)
...
ggsave("Extened_figure4A.pdf", width = 18, height = 4)
####################################################################################################
###########methy_cv.txt
...
m_l_data <- read.table("figure_d.txt", header = TRUE)
names(m_l_data) <- c("group", "gene", "degree", "score")
...
ggsave("Figure_4d.pdf", width = 16, height = 4)
###################################################################################################
