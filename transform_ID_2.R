...
setwd("/data5/galaxy/project/lncRNA_analysis/m6a_expression/unm6a_lncRNA")
...
geneEnsemble_to_symbol <- function(in_file){
  ...
  df_sort = df[ordered(df$Gene_ID), ]
  write.table(unique(df_sort), paste("symbol", in_file, sep = "_"), sep = "\t", row.names = FALSE, quote = FALSE)
  # print(head(df))
}
