...
#####################################################################################
library(effsize)
setwd("/home/galaxy/project/revised_version/project/CpG_density/m6a_vs_free")
######################################
total_df = read.table("m6a_vs_free.txt", sep = "\t", header = FALSE)
colnames(total_df) <- c("Organ", "Type", "value")
head(total_df)
...
calculate_effectiveSize <- function(tissue){
  ...
  GroupA <- df[df$Type == "m6a", ][, 3]
  GroupB <- df[df$Type == "free", ][, 3]
...
#####################
library(effsize)
setwd("/home/xialj/data6/xlj/Gene_Expression_divergence/gene_methylation_level/Rank_expCV/")
df = read.table("data_for_plot", sep = "\t", header = TRUE)
head(df)
GroupA <- df[df$type == "High", ][, 3]
GroupB <- df[df$type == "Low", ][, 3]
result = cohen.d(GroupA, GroupB, pooled = TRUE, conf.level = 0.95)
print(result)
################################
...
#####################
library(effsize)
setwd("/home/xialj/data6/xlj/Gene_Expression_divergence/gene_methylation_level/Rank_m6A_level/lincRNA/")
############
# d 
df = read.table("data_for_plot", sep = "\t", header = TRUE)
head(df)
